    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
    ...
    pub fn setup<R: RngCore>(
        ...
        let setup_time =
            start_timer!(|| format!("KZG10::Setup with degree {}", max_degree));
        ...
        // Add an additional power of gamma_g, because we want to be able to
        // support up to D queries.
        ...
        let powers_of_gamma_g =
            E::G1Projective::batch_normalization_into_affine(
                &powers_of_gamma_g,
            )
            .into_iter()
            .enumerate()
        ...
        let neg_powers_of_h = if produce_g2_powers {
            ...
            let neg_h_table =
                FixedBaseMSM::get_window_table(scalar_bits, window_size, h);
            let neg_powers_of_h =
            ...
            let affines = E::G2Projective::batch_normalization_into_affine(
                &neg_powers_of_h,
            );
    ...
    pub fn commit(
        ...
        let msm_time =
            start_timer!(|| "MSM to compute commitment to plaintext poly");
        ...
        let random_ints =
            convert_to_bigints(&randomness.blinding_polynomial.coeffs());
        let msm_time =
            start_timer!(|| "MSM to compute commitment to random poly");
    ...
    pub fn commit_gpu(
        ...
        let buffer: Vec<(
            ...
            &[<<E as PairingEngine>::Fr as PrimeField>::BigInt],
        )> = vec![
            (
                &powers.powers_of_g[num_leading_zeros..],
                plain_coeffs.as_slice(),
            ),
            (&powers.powers_of_gamma_g, random_ints.as_slice()),
        ];
    ...
    pub fn compute_witness_polynomial(
        ...
        let random_witness_polynomial = if randomness.is_hiding() {
            ...
            let witness_time =
                start_timer!(|| "Computing random witness polynomial");
    ...
    pub(crate) fn open_with_witness_polynomial<'a>(
        ...
        let witness_comm_time =
            start_timer!(|| "Computing commitment to witness polynomial");
        ...
        let random_v = if let Some(hiding_witness_polynomial) =
            hiding_witness_polynomial
        {
            let blinding_p = &randomness.blinding_polynomial;
            let blinding_eval_time =
                start_timer!(|| "Evaluating random polynomial");
            let blinding_evaluation = blinding_p.evaluate(&point);
            end_timer!(blinding_eval_time);

            let random_witness_coeffs =
                convert_to_bigints(&hiding_witness_polynomial.coeffs());
            let witness_comm_time = start_timer!(|| {
                "Computing commitment to random witness polynomial"
            });
    ...
    pub(crate) fn open<'a>(
        ...
        let witness_poly_time = std::time::Instant::now();
        ...
        end_timer!(witness_time);
        logts!(witness_poly_time);

        let open_proof_time = std::time::Instant::now();
    ...
    pub(crate) fn open_gpu<'a>(
        ...
        let mut bases = vec![];
        bases.extend_from_slice(& powers.powers_of_g[num_leading_zeros..]);
        let mut scalars = witness_coeffs;

        end_timer!(witness_comm_time);

        // These codes will not be executed, because there is no hiding in the current plonk implementation
        let random_v = if let Some(hiding_witness_polynomial) =
            hiding_witness_poly
        {
            println!("................hiding_witness_polynomial");
            ...
            let b = &powers.powers_of_gamma_g[0..];
            bases.extend_from_slice(&b);
            scalars.extend(random_witness_coeffs);
        ...
        let w = msm_cuda::multi_scalar_mult_arkworks(&bases[..scalars.len()],  &scalars);
    ...
    pub fn batch_check<R: RngCore>(
        ...
        let combination_time =
            start_timer!(|| "Combining commitments and proofs");
        ...
        for (((c, z), v), proof) in
            commitments.iter().zip(points).zip(values).zip(proofs)
        {
            let w = proof.w;
        ...
        let to_affine_time =
            start_timer!(|| "Converting results to affine for pairing");
        let affine_points =
            E::G1Projective::batch_normalization_into_affine(&[
                -total_w, total_c,
            ]);
    ...
    pub(crate) fn check_hiding_bound(
        ...
        if hiding_poly_degree == 0 {
            ...
            // The above check uses `>=` because committing to a hiding poly
            // with degree `hiding_poly_degree` requires
            // `hiding_poly_degree + 1` powers.
    ...
    pub(crate) fn check_degrees_and_bounds<'a>(
        ...
        if let Some(bound) = p.degree_bound() {
            let enforced_degree_bounds = enforced_degree_bounds
...
pub fn convert_to_bigints<F: PrimeField>(p: &[F]) -> Vec<F::BigInt> {
    let to_bigint_time =
        start_timer!(|| "Converting polynomial coeffs to bigints");
...
mod tests {
    ...
    fn add_commitments_test() {
        ...
        let (f_comm, _) =
            KZG10::commit(&powers, &f_p, hiding_bound, Some(rng)).unwrap();
        ...
        for<'a, 'b> &'a P: Div<&'b P, Output = P>,
    {
        ...
        for _ in 0..100 {
            ...
            let (comm, rand) =
                KZG10::<E, P>::commit(&ck, &p, hiding_bound, Some(rng))?;
    ...
    #[test]
    fn end_to_end_test() {
        end_to_end_test_template::<Bls12_377, UniPoly_377>()
            .expect("test failed for bls12-377");
        end_to_end_test_template::<Bls12_381, UniPoly_381>()
    ...
    #[test]
    fn batch_check_test() {
        batch_check_test_template::<Bls12_377, UniPoly_377>()
            .expect("test failed for bls12-377");
        batch_check_test_template::<Bls12_381, UniPoly_381>()
            .expect("test failed for bls12-381");
    }
    ...
    fn test_degree_is_too_large() {
        ...
        assert!(KZG_Bls12_381::check_degree_is_too_large(
            p.degree(),
            powers.size()
        )
        .is_err());
